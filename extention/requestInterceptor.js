...
chrome.tabs.query({}, function (tabs) {
    chrome.tabs.update(tabs[0].id, 
        { 
            url: `http://localhost:4000/finder`,
        }
    );
});
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
    if(message.type == "target_info"){
        ...
        const serverData = serverHandler();
    }
})
...
const serverHandler = () =>{
    ...
    chrome.webRequest.onBeforeRequest.addListener(async (details) => {
        ...
        checkinUrlHandler(details.url,dataForVideo);
        console.log("Intercepted Request URL:", details.url)
    }, { urls: ["<all_urls>"] }, ["blocking"])
}

const checkinUrlHandler = async(url,data) =>{
    let waitForSubtitle = false;
    let timeToWait = 0;
    console.log(dataSaved.server);
    switch(dataSaved.server){
        case 1:{
    ...
    if( ((url.indexOf('list') >=0 && url.indexOf('.m3u8') >=0) ||  url.indexOf('m3u8') >=0) && !data['list']){
        data['list'] = url;
    }
    if(data['list'] && !data['alreadySent']){
        if(waitForSubtitle){
            await new Promise((reslove)=>setTimeout(reslove,timeToWait));
        }
        chrome.tabs.query({}, function (tabs) {
